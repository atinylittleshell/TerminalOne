...
const describeKey = (event: KeyboardEvent) => {
  ...
  const normalizedKey = KEY_REPLACEMENTS[key] || key;
  const modifiers = [altKey && 'alt', ctrlKey && 'ctrl', metaKey && 'meta', shiftKey && 'shift'].filter(Boolean);

  return normalizeKeyDescriptor(modifiers.length > 0 ? `${modifiers.join('+')}+${normalizedKey}` : normalizedKey);
};
...
const keyHandler = (event: KeyboardEvent, config: KeybindConfig) => {
  ...
  console.log('RAW', type, normalizedKey, keyDescriptor);
  ...
  if (MODIFIER_KEYS.has(normalizedKey.toLowerCase())) {
    // modifier keys should always be passed through to the terminal
    console.log('TERM-MODIFIER', type, keyDescriptor);
    return true;
  }
  ...
  if (type === 'keydown') {
    ...
    if (isLeaderActive(event)) {
      ...
      if (command) {
        ...
        lastLeaderKeyEvent = null;
        keyState[normalizedKey] = true;
        console.log('TERM-NOT-BOUND', type, keyDescriptor);
        return true;
      }
    } else {
      ...
      keyState[normalizedKey] = true;
      console.log('TERM-NO-LEADER', type, keyDescriptor);
      return true;
    }
  }

  const isSameKeyDown = Boolean(keyState[normalizedKey]);
  if (type === 'keyup') {
  ...
  console.log(isSameKeyDown ? 'TERM-FOLLOW-KEYDOWN' : 'HOST', type, keyDescriptor);
  return isSameKeyDown;
};

const normalizeKeyDescriptor = (keyDescriptor: string) => {
  const items = keyDescriptor.split('+').map((item) => KEY_REPLACEMENTS[item] || item);
  const modifiers = _.uniq(
    items
      .filter((item) => MODIFIER_KEYS.has(item.toLowerCase()))
  ...
  const keys = items
    .filter((item) => !MODIFIER_KEYS.has(item.toLowerCase()))
    .map((item) => {
      // except for uppercased A-Z, all other keys should be normalized to lowercase
